...
O Asaas recebe o valor nominal da cobrança em `value` e somente o desconto monetário já congelado pelo Disparador em `discount.value`, como `FIXED` válido até o vencimento. O provedor não recebe percentuais, FIFO, créditos ou regras de ciclo. `COB_Valor` continua representando internamente o valor líquido para preservar seus consumidores atuais.
