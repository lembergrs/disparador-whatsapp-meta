...
## Registro operacional do Phone Number pós-Embedded Signup

Quando a Meta retorna o Phone Number com `status=PENDING`, o Disparador deve manter a conta como `requer_acao` mesmo que `code_verification_status=VERIFIED` e o nome esteja disponível. Nesse estado, WABA ID, Phone Number ID, token e metadados são preservados para permitir concluir o registro sem refazer o Embedded Signup.

A ação **Concluir registro** solicita o PIN de verificação em duas etapas do WhatsApp. Esse PIN:

- tem exatamente 6 dígitos;
- é definido/informado pelo cliente/parceiro na Meta;
- não é o OAuth code;
- não é o OTP de SMS/voz;
- não é salvo pelo Disparador;
- não deve aparecer em logs, flash messages ou respostas de erro.

O backend chama `POST /{PHONE_NUMBER_ID}/register` com:

```json
{
  "messaging_product": "whatsapp",
  "pin": "<PIN_DE_6_DIGITOS>"
}
```

Após o register, o Disparador reconsulta o Phone Number, atualiza metadados operacionais e só muda para `conectado` quando a Meta não indicar pendência/bloqueio. Apenas então garante `subscribed_apps` e inicia o trial. Se o número continuar `PENDING` ou se a Meta rejeitar o PIN, a conta permanece `requer_acao` e o cliente pode tentar novamente com o PIN correto ou usar **Atualizar status** para reconsultar a Meta sem informar PIN.
