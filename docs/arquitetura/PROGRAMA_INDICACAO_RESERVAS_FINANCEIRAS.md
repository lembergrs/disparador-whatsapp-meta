...
`IndicacaoDescontoService::prepararDesconto()`:

1. valida referência, ciclo, cliente e valor-base;
2. serializa a referência com lock nomeado no MariaDB;
3. retorna snapshots existentes se a referência já foi preparada;
4. seleciona com lock somente créditos `liberado`, em `ICR_LiberadoEm ASC, ICR_ID ASC`;
5. limita a seleção aos meses do ciclo;
6. calcula cada desconto histórico;
7. cria reservas e muda cada crédito para `reservado`;
8. confirma tudo em uma transação.
...
`liberarReservas()` exige motivo e muda todas as reservas para `liberada` e todos os créditos para `liberado`. `ICR_LiberadoEm` usa `COALESCE`, preservando a data original e a posição histórica no FIFO. Repetição é idempotente; uma referência já liberada não pode ser confirmada.
