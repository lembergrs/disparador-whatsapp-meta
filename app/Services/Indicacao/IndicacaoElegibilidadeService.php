...
class IndicacaoElegibilidadeService
{
    ...
    public static function padrao(): self
    {
        ...
        $indicacaoService = new IndicacaoService($indicacoes, new IndicacaoCampanha($db), new IndicacaoCodigo($db), $auditoria, null, $db);
        $creditoService = new IndicacaoCreditoService($creditos, $indicacoes, $auditoria, null, $db);
    ...
    public function confirmarPrimeiroPagamento($clienteIndicadoId, \DateTimeInterface $pagoEm): array
    {
        ...
        return $this->transacao(function() use ($clienteIndicadoId, $pagamento){
            ...
            if($indicacao['IND_Status'] === 'aprovada'){
                ...
                if(!$credito || $credito['ICR_Status'] !== 'liberado') throw new \DomainException('Indicação aprovada em estado inconsistente.');
            ...
            if($indicacao['IND_Status'] === 'em_confirmacao'){
                ...
                $tarefa = $this->agendarConfirmacao((int)$indicacao['IND_ID'], new \DateTimeImmutable($indicacao['IND_ConfirmacaoAte']));
                return ['id'=>(int)$indicacao['IND_ID'], 'idempotente'=>true, 'tarefa_id'=>(int)$tarefa['id'], 'confirmacao_ate'=>$indicacao['IND_ConfirmacaoAte']];
            }
            ...
            $this->indicacaoService->confirmarPagamento((int)$indicacao['IND_ID'], $pagamento, $confirmacaoAte);
            $tarefa = $this->agendarConfirmacao((int)$indicacao['IND_ID'], $confirmacaoAte);
            $this->indicacaoService->iniciarConfirmacao((int)$indicacao['IND_ID']);
    ...
    public function processarConfirmacao($indicacaoId): array
    {
        try{
            return $this->transacao(function() use ($indicacaoId){
                ...
                if($indicacao['IND_Status'] !== 'em_confirmacao') throw new TaskPermanentFailureException('Estado da indicação incompatível com confirmação.');
                if(empty($indicacao['IND_PagamentoConfirmadoEm']) || empty($indicacao['IND_ConfirmacaoAte'])) throw new TaskPermanentFailureException('Indicação sem marcos de confirmação.');
                if($credito) throw new TaskPermanentFailureException('Crédito existente para indicação ainda não aprovada.');

                $confirmacaoAte = new \DateTimeImmutable($indicacao['IND_ConfirmacaoAte']);
                if($this->agora() < $confirmacaoAte) throw new TaskRetryException('Janela de confirmação ainda não concluída.');

                $this->indicacaoService->aprovar((int)$indicacao['IND_ID']);
                $creditoId = $this->creditoService->criarPorIndicacao((int)$indicacao['IND_ID']);
                $this->creditoService->alterarStatus($creditoId, 'em_confirmacao');
                $this->creditoService->alterarStatus($creditoId, 'liberado');
    ...
    {
        return $this->scheduler->agendar(self::TIPO_TAREFA, ['indicacao_id'=>(int)$indicacaoId], $confirmacaoAte, self::CHAVE_PREFIXO . (int)$indicacaoId);
    }
