...
class IndicacaoCreditoReservaService
{
    ...
    public function restabelecer(array $reservas):void{$this->transacao(function()use($reservas){foreach($reservas as $reserva){IndicacaoReservaStatusService::validar($reserva['ICRR_Status'],'reservada');$this->creditos->alterarStatus($reserva['ICR_ID'],'reservado',null,'retry da referência financeira');if(!$this->reservas->restabelecer($reserva['ICRR_ID']))throw new \RuntimeException('Reserva alterada concorrentemente.');$this->auditoria->registrar('credito_reserva',$reserva['ICRR_ID'],'reserva_restabelecida','liberada','reservada','retry da referência financeira');}});}
