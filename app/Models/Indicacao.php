...
class Indicacao
{
 ...
 public function buscar($id,$lock=false){$s=$this->db->prepare('SELECT * FROM indicacoes WHERE IND_ID=?'.($lock&&$this->driver()==='mysql'?' FOR UPDATE':''));$s->execute([(int)$id]);return $s->fetch(PDO::FETCH_ASSOC)?:null;}
 public function buscarPorIndicado($clienteId,$lock=false){$s=$this->db->prepare('SELECT * FROM indicacoes WHERE CLI_Indicado_ID=?'.($lock&&$this->driver()==='mysql'?' FOR UPDATE':'').' LIMIT 1');$s->execute([(int)$clienteId]);return $s->fetch(PDO::FETCH_ASSOC)?:null;}
 public function confirmarPagamento($id,$pagoEm,$confirmacaoAte): bool {$s=$this->db->prepare("UPDATE indicacoes SET IND_Status='pagamento_confirmado',IND_PagamentoConfirmadoEm=?,IND_ConfirmacaoAte=? WHERE IND_ID=? AND IND_Status='aguardando_pagamento'");$s->execute([$pagoEm,$confirmacaoAte,(int)$id]);return $s->rowCount()===1;}
