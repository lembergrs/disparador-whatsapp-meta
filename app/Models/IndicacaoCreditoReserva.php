...
class IndicacaoCreditoReserva
{
    ...
    public function restabelecer($id){$s=$this->db->prepare("UPDATE indicacao_credito_reservas SET ICRR_Status='reservada',ICRR_LiberadoEm=NULL WHERE ICRR_ID=? AND ICRR_Status='liberada'");$s->execute([(int)$id]);return $s->rowCount()===1;}
