...
class IndicacaoCredito
{
 ...
 public function buscarPorIndicacao($indicacaoId,$lock=false){$s=$this->db->prepare('SELECT * FROM indicacao_creditos WHERE IND_ID=?'.($lock&&$this->driver()==='mysql'?' FOR UPDATE':'').' LIMIT 1');$s->execute([(int)$indicacaoId]);return $s->fetch(PDO::FETCH_ASSOC)?:null;}
 ...
 public function selecionarDisponiveisFifo($cliente,$limite,$lock=true): array {$limite=max(1,min(12,(int)$limite));$forUpdate=$lock&&$this->driver()==='mysql'?' FOR UPDATE':'';$s=$this->db->prepare("SELECT * FROM indicacao_creditos WHERE CLI_Indicador_ID=? AND ICR_Status='liberado' ORDER BY ICR_LiberadoEm ASC,ICR_ID ASC LIMIT {$limite}{$forUpdate}");$s->execute([(int)$cliente]);return $s->fetchAll(PDO::FETCH_ASSOC);}
 public function status($id,$anterior,$novo): bool {$campos=['bloqueado'=>'ICR_BloqueadoEm','cancelado'=>'ICR_CanceladoEm','expirado'=>'ICR_ExpiradoEm','utilizado'=>'ICR_UtilizadoEm'];$extra=isset($campos[$novo])?",{$campos[$novo]}=CURRENT_TIMESTAMP":'';if($novo==='liberado')$extra=',ICR_LiberadoEm=COALESCE(ICR_LiberadoEm,CURRENT_TIMESTAMP)';$s=$this->db->prepare("UPDATE indicacao_creditos SET ICR_Status=?{$extra} WHERE ICR_ID=? AND ICR_Status=?");$s->execute([$novo,(int)$id,$anterior]);return $s->rowCount()===1;}
